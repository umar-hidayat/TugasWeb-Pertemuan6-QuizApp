...
- **Single Page Application (SPA):** Transisi antar soal dan visualisasi hasil tanpa *reload* halaman.
- **Event Delegation Pattern:** Penanganan interaksi tombol jawaban terpusat menggunakan `e.target.closest()` untuk efisiensi memori.
- **XSS Prevention:** Seluruh manipulasi konten DOM menggunakan `textContent` dan `createElement` (bebas dari bahaya `innerHTML` pada data dinamis).
- **Persistent High Score:** Menyimpan rekor nilai tertinggi ke penyimpanan lokal browser menggunakan `localStorage` API (`JSON`/`Number` parsing).
